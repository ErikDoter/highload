...
Характеристики возьмем следующие:
...
Нам достаточно 4 сервера, но для надежности возьмем 8 c характеристиками:
...
#### БД
Исходя из пункта 4 было обнаружено, что нужно использовать 22 сервера. Возьмем следующие характеристики

| CPU(cores) | RAM(GB) | SSD(TB) |
|------------|---------|---------|
|   16       | 16      | 20    |

#### Общая таблица серверов выглядит следующим образом:

| Узел | CPU (cores) | RAM (гб) | Диск SSD (гб) | Количество |
|:------:|:------:|:------:|:------:|:------:|
| Бэкенд | 16 | 16 | 512 | 10 |
| БД | 16 | 16 | 20*1024 | 22 |
| Фротенд | 32 | 32 | 512 | 8 |
| Proxy перед S3 | 32 | 128 | 16*1024 | 10 |